...
function sgm()something.pgm = currentGameMode currentGameMode = updateSomething end
	...
	cmenu = {
		...
		items={
			...
			{text = "Delete", callback = function(f)
				showPopup(f.name, "Really delete \""..f.name.."\"?",
					{
						{sprite = "MENU_NO", callback = function()
							return true
						end},
		...
		if selected then
			...
			if keyPressed.LBUTTON then
				...
				if v.info.type == "directory" then
					so.path = resolvePath(so.path..v.name).."/" if so.path=="//"then so.path="/"end
					so.files = reloadSomething(so,so.path)
				else
...
function reloadSomething(so,path)
	...
	files = {}
	if path ~= "/" then table.insert(files, {name = "..", info = {type = "directory"}}) end
