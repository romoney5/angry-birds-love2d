...
function res.createBitmapFont(font,silent)
	...
	if not silent then print("Loading font file: "..font) end
	if not checkDirectory(font) then font = fontPath.."/1024x768/"..font:match("([^/]+)$") end --attempt to use pc font if current doesn't exist
...
function res.drawString(group, text, x, y, aligny, alignx)
	...
	if font then
		...
		love.graphics.print(text, x, y+ay) love.graphics.setBlendMode(bm,am)
	end
...
function drawUITextNative(self, x, y, scale_x, scale_y, angle, hover_scale)
	...
	love.graphics.translate(_G.math.floor(self.x*hs + x), _G.math.floor(self.y*hs + y))
...
function res.getStringWidth(text,font)
	...
	if font then
		...
		for c in text:gmatch(".") do
			local char = font.chars[string.format("%04x", string.byte(c))]
			if char then
				i = i + char.width + font.tracking highscore = math.max(highscore,i)
			elseif c == "\n" then i = 0 end
		end
		return highscore
	else
